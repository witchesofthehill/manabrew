...
`ReportGameOutcome` is the second: how a game ended, from the seat that ran the engine. The relay used to read `gameOver`, `winnerId` and concessions out of the state stream it forwarded, which stopped being true once game traffic could go peer to peer (`docs/TRANSPORT.md`), so it no longer reads state for the outcome at all. The host files the outcome in engine slots (`GameOutcomeReport`), the relay maps slots to the usernames it seated (`GameReplayCache::username_for_slot`) and never takes a name from the client, and only the room's host may file, only for the `game_id` the relay believes is running; anything else counts in `manabrew_relay_game_outcome_reports_total{kind="rejected"}` and is dropped silently. `game_ended.reported` says whether a report arrived; without one the reason is the room lifecycle's fallback (`host_ended`, `host_lost`, …) and `game_over` is false because nobody said otherwise, not because the game was abandoned. The browser host sends it from `useGameEventListeners`, the hosted node from `spawn_game_over_forwarder` and its fatal path, both before `EndGame`.
