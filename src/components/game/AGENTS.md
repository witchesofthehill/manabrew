...
- **Pixi player HUD + on-grid zones**: the player panel is **all Pixi, always on** — there are no React player panels (the old `PlayerPanel`/`PlayerAvatar`/`BadgeOrbit`/`ZoneActionColumn` chain and the `pixiPlayerBar` pref were removed). It lives in `@/pixi/hud/`:
  - **`PlayerHudLayer`** owns one **`PlayerHudCapsule`** per player and is positioned by `BoardScene` (`setRect`). Each capsule is a compact, content-hugging **capsule HUD** (Option A): a circular **avatar** as the left-edge cap (photo → bot game-icon → name initial), then ♥ **life**, the floating **mana pool** (nonzero pips via `manaSymbolCache`), and any active **player/game badges** (glyphs via `gameIconTexture`, tinted per badge colour). The badge list + colours are built in `@/components/game/panels/playerHudBadges.ts` (`buildPlayerHudBadges`, mirroring the legacy React badge logic, plus self-only `useGameDevStore` overrides for preview) and passed on `PlayerHudSpec.badges`. Self capsule is bottom-left (`SELF_PLAYER_HUD_HEIGHT_PX`); opponent capsules sit across the top of their clip band, eased with the delimiters, and **collapse to an avatar sphere + life** (dropping mana/badges) when the field narrows to banner width. The opponent grid reserves the height via `RegionHost.getTopReserve`. Tap the avatar to target. Animations are **gsap** (life-change pop, badge fade-in, priority pulse) — no Pixi-ticker wiring. Driven by `BoardCanvas` `playerBars` / `showPlayerBars`; specs built in `GameBoard.playerBarSpecs`.
