...
// Generated by build.rs: every tracked deploy-config file, embedded at build
// time. The binary built at a release tag carries that tag's config.
include!(concat!(env!("OUT_DIR"), "/config_embed.rs"));

// Set by CI (XTASK_BUILD_TAG=$GITHUB_REF_NAME) so the shipped binary knows
// which release it belongs to; None on plain dev builds.
...
// command exists to close (issue #512 gap 1). The shipped binary embeds its
// build tag, so the check needs no checkout; a dev build falls back to
// describing the checkout it embeds config from.
fn assert_tag_checkout(root: &Path, opts: &Opts) -> Result<()> {
    let source = match BUILD_TAG {
    ...
    let msg = format!(
        "this binary carries config from `{source}`, not `{}` — it would not match the images",
        opts.tag
    );
    if opts.allow_untagged || matches!(opts.mode, Mode::Only(_)) {
        ...
        bail!("{msg}; use that release's binary (or pass --allow-untagged)");
    }
}
...
// installed clients are actually running against, fetched with curl so no git
// history is needed. New side: the manifest embedded in this binary.
fn gate(root: &Path, tag: &str) -> Result<bool> {
    let early = match released_manifest(root) {
        ...
        Some(released) => match incompatible_crate(&released, &embedded_manifest()?)? {
            Some(reason) => {
...
fn embedded_manifest() -> Result<serde_json::Value> {
    let (_, bytes) = CONFIG_FILES
        .iter()
        .find(|(p, _)| *p == "ops/manifest.json")
        .context("ops/manifest.json missing from the embedded config")?;
    serde_json::from_slice(bytes).context("parsing the embedded ops/manifest.json")
}
...
// Ships the config embedded in this binary (tracked files only, captured at
// build time — the gitignored data dirs under ops/ can't be in the set, so
// the sync structurally can't touch them; no --delete for the same reason).
// Returns rsync's itemized change list: what ACTUALLY changed on the box,
// which is drift-proof where a commit-range diff is not.
fn sync_config(root: &Path, opts: &Opts) -> Result<String> {
    let staged = std::env::temp_dir().join(format!("manabrew-deploy-{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&staged);
    for (path, bytes) in CONFIG_FILES {
        let dest = staged.join(path);
        std::fs::create_dir_all(dest.parent().unwrap())?;
        std::fs::write(&dest, bytes)?;
    }
    println!(
        "📤 syncing {} embedded config files ({}) to {}:{}",
        CONFIG_FILES.len(),
...
fn head_label(root: &Path, opts: &Opts) -> String {
    match &opts.mode {
        Mode::Staging { .. } => run(root, "git", &["rev-parse", "--short", "HEAD"])
            .map(|s| s.trim().to_string())
            .unwrap_or_else(|_| BUILD_TAG.unwrap_or("this build").to_string()),
        _ => opts.tag.clone(),
    }
}
...
fn deploy(root: &Path, opts: &Opts) -> Result<()> {
    ...
    let itemized = sync_config(root, opts)?;
