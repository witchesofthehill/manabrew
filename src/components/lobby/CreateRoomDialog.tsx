    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent
        className={cn(
          ...
          showPicker ? "max-w-4xl" : "max-w-2xl",
        )}
      >
        <div className="px-6 pt-6 pb-4 shrink-0">
          <DialogTitle className="text-lg">Create Table</DialogTitle>
          <DialogDescription className="text-sm text-muted-foreground">
            Choose what you want to play. Your table opens as soon as it is created.
          </DialogDescription>
        </div>
          ...
          >
            ...
            <details className="rounded-xl border border-border/70 bg-muted/20 px-4 py-3">
              ...
              <p className="mt-1 text-xs text-muted-foreground">
                Table name, privacy, game engine, and reconnect behavior.
              </p>
                    ...
                    {kind === "match" &&
                      (isTauri && forgeRoomAvailable ? (
                        ...
                        >
                          ...
                          <span className="text-[10px] text-muted-foreground leading-tight">
                            Full card support, hosted in-app on this device. Others join from the
                            lobby.
                          </span>
                        </button>
                      ) : !isTauri ? (
                    ...
                    <div className="flex items-center gap-2">
                      {RECONNECT_TIMEOUT_OPTIONS.map((seconds) => (
                        <button
                          key={seconds}
                          type="button"
                          onClick={() => setReconnectTimeoutS(seconds)}
                          className={cn(
