...
- **Layout** is a single arrangement: the local player along the bottom (`RegionOrientation` `bottom`), opponents across the top (`top`). Each opponent field's `rect` is the **fixed full-width play area** (`x = 0`, full canvas width) — the grid, card positions, felt and playmat are all drawn from it and **never move or reflow**. The state is the **`count - 1` delimiter positions** between fields (one number each, fractions of width, ascending), and the _only_ thing they change is each field's **clip band** (`clipX`/`clipWidth`), applied by `BoardRegion.updateClip` as a Pixi mask on the region container. The bands **TILE the canvas edge-to-edge — they never overlap** — which is what prevents one field's cards from bleeding into another's (clipping alone can't, because the felt is semi-transparent; the felt is drawn over the fixed `rect` and only the mask moves). Delimiters are written by the **same operation** from two sources: auto-focus (`GameBoard` effect expands the active opponent's band to ~full canvas on their turn, the rest to a `COLLAPSED_OPPONENT_WIDTH_PX` banner) and the manual **resize grips** (drag one delimiter to its cursor position, clamped between its neighbours). Because the rect is fixed, dragging a delimiter only un-clips more of a field's existing cards — nothing recomputes or moves. `GameBoard` anchors its React player panels to each field's visible band (`clipX`, not `rect.x` — every field's `rect.x` is 0) via `BoardCanvas`'s `onLayout`. Every player's React panel is measured (relative to the board) and pushed to its region via `BoardScene.setPlayerBlockers` as a canvas-space keep-out, so battlefield cards never lay out under that player's own zones/avatar — converted into the region's local space (`collectLocalBlockers`).
