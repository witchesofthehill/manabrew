...
import type { ChooseRollSwapValueInput, ChooseRollSwapValueOutput } from "@/protocol";
...
export function ChooseRollSwapValueModal({
  input,
  respond,
}: PromptProps<ChooseRollSwapValueInput, ChooseRollSwapValueOutput>) {
      ...
      <div role="dialog" aria-modal="true" aria-labelledby="choose-roll-swap-value-title">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 id="choose-roll-swap-value-title" className="font-semibold text-base">
                Exchange roll {input.currentResult}
              </h2>
        ...
        <div className="p-4 grid grid-cols-2 gap-3">
          <Button
            variant="outline"
            size="lg"
            onClick={() => respond({ type: "rollSwapValueDecision", choice: "power" })}
          >
            <span className="flex flex-col items-center">
              <span className="text-xs uppercase tracking-wide text-muted-foreground">Power</span>
              <span className="text-xl font-bold">{input.power}</span>
            </span>
          </Button>
          <Button
            variant="outline"
            size="lg"
            onClick={() => respond({ type: "rollSwapValueDecision", choice: "toughness" })}
          >
            <span className="flex flex-col items-center">
              <span className="text-xs uppercase tracking-wide text-muted-foreground">
                Toughness
              </span>
              <span className="text-xl font-bold">{input.toughness}</span>
            </span>
          </Button>
        </div>
      </div>
