...
The inbound direction is one union, `ClientToServerMessage` (`manabrew-protocol` transport): `Response { action }` (answers the pending prompt, strictly paired) or `Directive { directive }` (`DirectiveInput` — an out-of-band, fire-and-forget player instruction, never paired with a prompt; currently only `concede`). Every channel into the engine carries this union — the SAB seat channels, the node's mpsc seat channels. **A directive never answers a prompt and no layer ever fabricates an answer on a player's behalf**: the agent's recv loop absorbs directives and keeps waiting — the pending prompt stays pending and the player (still a participant) answers it. A concede directive registers the concession (`PromptAgent.conceded`) and is honored at the seat's next priority window (`choose_action` returns `EnginePlayerAction::Concede` without prompting); a directive arriving while the priority prompt itself is open _is_ the decision — conceding is a legal priority action. Send via `runtime.api.sendDirective({ playerSlot, directive })`: a relay guest broadcasts it as a `{ kind: "directive" }` envelope to whoever hosts the engine (WASM host or self-hosted node); the engine host delivers it into the seat's channel itself (`WorkerBridge.deliverLocalDirective` / `deliverRemoteDirective` — the SAB holds one slot, so delivery waits until the engine is blocked on that seat's prompt, then the prompt is still relayed for the player to answer).
