...
- Free layout offers three pointer handles in a row at the bottom-centre of each tile: rotate (tap → snap to next 90°, drag → continuous free rotation), scale (tap → reset to 1×, drag → continuous resize relative to distance from tile centre, clamped between 0.55× and 2×, persisted as `freeLayout.scale`), and grip (move-only). Handle drags are continuous (no per-step snapping) so they feel smooth. The card body itself is also a move target: capture-phase pointer handlers on the FreeTile wrapper start a maybe-drag on `pointerdown`, promote it to a real drag after 8px of motion, dispatch a `pointercancel` on the inner press-hold so the tap doesn't fire, and skip entirely when the press began inside a `[data-companion-handle]` element. Free-rotation angles outside ±90° intentionally let the rotated tile spill over its bounding box — the board container has `overflow-hidden` so the spill doesn't break layout. Two fingers on the body do native pan + zoom + rotate at once: `FreeTile` tracks pointers in an `activePointers` map; the second `pointerdown` aborts the in-flight life tap/drag and seeds `twoFinger` (the two ids, their last positions, and a live `pos` ref). Each move applies `twoFingerTransform` — a similarity transform that maps the previous finger pair to the current one and pivots the tile centre around the finger centroid, so the content stays under the fingers. It's continuous (no mid-gesture snapping); `settlePosition` only magnets a near-straight rotation / near-1× scale into place when the gesture ends. Working off the `twoFinger.pos` ref (not the `position` prop) avoids stale-render drift. The body keeps `touch-none` so the browser doesn't claim the second finger for page pinch-zoom.
