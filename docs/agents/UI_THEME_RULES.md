...
7. **Pair foregrounds with fills.** App actions use the corresponding `*-foreground`; gameplay actions use `promptForeground.*`. `cardSelection` is distinct from `cardRing` and from the app's `selection` token. Root game-token names must not collide with app CSS variables.
8. **Conditional classes use `cn()`**, never template literals. Static strings only — Tailwind's JIT cannot detect `bg-${color}-400`.
