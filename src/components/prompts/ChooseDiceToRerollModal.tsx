...
import type { ChooseDiceToRerollInput, ChooseDiceToRerollOutput } from "@/protocol";
...
export function ChooseDiceToRerollModal({
  input,
  respond,
}: PromptProps<ChooseDiceToRerollInput, ChooseDiceToRerollOutput>) {
  const sourceCard = usePromptSourceCard();
  const accentColor = useTheme().gameTheme.playerColors.self;
  const [selected, setSelected] = useState<Set<number>>(new Set());

  const dice: SelectableDie[] = input.rolls.map((value, index) => ({ index, value }));
  ...
  const handleConfirm = () => {
    ...
    respond({ type: "diceToRerollDecision", rolls: chosen });
  };
      ...
      <div role="dialog" aria-modal="true" aria-labelledby="choose-dice-reroll-title">
        ...
        <Modal.Footer>
          <Button
            variant="outline"
            size="sm"
            onClick={() => respond({ type: "diceToRerollDecision", rolls: [] })}
          >
            Skip
          </Button>
