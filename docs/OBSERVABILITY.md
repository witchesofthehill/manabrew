...
**`turnaround_*` is split into `client_work_*` and `reply_wait_*`.** `client_work_*` is the time the client spent handling the frames that arrived between its answer and the next prompt: parsing and applying each one, the prompt's own frame included. `reply_wait_*` is the rest: the server, the hop, the player's link, the transfer, and whatever the engine and the other seats did between frames. The two sum to `turnaround_*` per decision. Until 2026-09-17 the cut was at the first frame after the answer, and in a room that frame is the echo of the player's own action, so an opponent's whole turn landed in `client_work_*`: it scaled with seat count and hit tens of seconds at p90. Rows from clients older than the release carrying this fix (2026-09-17) carry that reading; solo forge-wasm rows were unaffected (one frame, ~17ms). Null from clients that predate the split and from engines with no frame boundary; for the browser Forge engine the frame is the seat read, so `reply_wait_*` there is engine time plus the seat drain, not a wire.
