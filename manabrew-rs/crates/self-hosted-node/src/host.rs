...
async fn handle_server_message(
    ...
    match message {
        ...
        ServerMessage::RoomTransport {
            ...
            if let Some(plane) = direct {
                // A host binds without a relay, because ours admits nobody
                // without a room token and the token arrives here. Re-run on
                // every broadcast so an expiring token is replaced before it
                // refuses the next reconnect; only the first adoption changes
                // the address, and only that needs announcing again.
                if plane
