...
| `audit-prompt-contract.mjs`                         | **Static** contract check: parses the Rust `AgentPromptInner` enum vs TS `PromptType`/`HANDLED_PROMPT_TYPES` and reports mismatches.                                                                                               | —                                              |
| `setup-windows-runner.ps1`, `setup-linux-runner.sh` | One-time CI runner provisioning.                                                                                                                                                                                                   | —                                              |

The **runtime** client-half prompt check is a vitest test (`src/stores/gameStore.constants.test.ts`, `yarn test`), not a script: it runs the rust `forge-agent-interface --bin emit_prompt_fixtures` bin on demand for one example of every `AgentPromptInner` variant, then replays them through the UI's `applyPrompt`, asserting every type is known/handled and ingests without throwing. No committed fixture — can't drift from the engine types.
