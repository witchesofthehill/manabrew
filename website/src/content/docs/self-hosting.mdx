...
<Tabs>
  ...
  <TabItem label="From source">
    ...
    Build the harness through `yarn build:harness` rather than Maven directly: it
    generates the harness's protocol classes before Maven runs, and a bare `mvn`
    call fails without them.
    ...
    The node runs `$JAVA_HOME/bin/java` when `JAVA_HOME` is set and the `java` on
    your `PATH` otherwise; set `JAVA_HOME` if that is not a JDK 17+.
