...
| `PulseRing.ts`    | The shared pulsating highlight ring (GSAP alpha yoyo) — use it for any new pulsing highlight (currently: hand playable via `CardSprite.setPlayableRing`). Add its `gfx` as a child; `show` is idempotent, so re-applying on every relayout never restarts the pulse. The stack's casting pulse (`StackCardSprite`) intentionally stays hand-rolled: it tweens the ring Graphics that also carries the seat stroke, so the whole border breathes as one — don't "unify" it onto PulseRing. |
| `config/`         | One `*.const.ts` per effect (`CRACKLE`, `DUST`, `FLASH`, `SHOCKWAVE`, `STOMP`, `EDGE_GLOW`, `DAMAGE_HIT`, `STAT_POP`, `SUMMONING_FILTER`, `PULSE_RING`), re-exported from `config/index.ts`. **All tunable numbers live here** — colors are still theme tokens at the call site. Tweak feel here, not in the effect body.                                                                                                                                                                 |
