...
export function DeckGridCard({
  ...
  const hasActions = Boolean(
    onPlaytest ||
    onViewInHub ||
    (!readOnly && (onPublish || onSaveToAccount || onRename || onDelete)),
  );
  ...
  return (
    <>
      <DeckTile
        ...
        titleClassName={getDeckNameColorClass(displayCards)}
        openLabel={`Open ${deck.deck.name}`}
        coverUrl={cover?.uris.art_crop}
        coverAlt={cover?.identity.name ?? deck.deck.name}
        formatId={deck.deck.format ?? "standard"}
        colorCost={getDeckColorCost(displayCards)}
        cardCount={displayCards.length}
        onOpen={onOpen}
        onPlay={onPlay}
        playLabel={`Play ${deck.deck.name}`}
        playing={playing}
        ...
        actions={
          hasActions ? (
            <div className="flex gap-1 opacity-0 transition-opacity group-focus-within:opacity-100 group-hover:opacity-100 pointer-coarse:opacity-100">
              {onPlaytest && (
                <Button
                  size="icon"
                  className="h-6 w-6"
