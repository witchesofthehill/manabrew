...
<Tabs>
  <TabItem label="Docker Compose">
    The image is published on GHCR and bundles everything: the node binary and
  ...
  <TabItem label="From source">
    You need a Rust toolchain, Node.js with Yarn, a JDK (17+), Maven, and a full checkout:
    ...
    Build the harness through `yarn build:harness`, not Maven directly — it
    generates the harness's protocol classes from the Rust protocol crate before
    Maven runs, and a bare `mvn` call fails without them.
    ...
    Point `JAVA_HOME` at your own JDK installation — the path above is a Debian
    example. On macOS use `JAVA_HOME="$(/usr/libexec/java_home -v 21)"`.
...
The published image compiles Forge ahead of time with
...
`SIGTERM` or `SIGINT` shuts the node down immediately: every room closes, and
...
With `restart: unless-stopped`, Docker restarts the container after the drain
