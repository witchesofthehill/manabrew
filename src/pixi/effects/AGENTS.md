...
| `config/`         | One `*.const.ts` per effect (`CRACKLE`, `DUST`, `STOMP`, `EDGE_GLOW`, `DAMAGE_HIT`, `STAT_POP`, `SUMMONING_FILTER`), re-exported from `config/index.ts`. **All tunable numbers live here** — colors are still theme tokens at the call site. Tweak feel here, not in the effect body. |
