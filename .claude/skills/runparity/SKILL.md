...
- Execute named tests from the regression suite in `manabrew-rs/crates/parity/regression.json`
...
- If **"all"** or **"regression"** is specified, read `manabrew-rs/crates/parity/regression.json` and run each entry sequentially with `yarn parity <name>`. Use a 300000ms timeout per test.
...
- Regression test definitions: `manabrew-rs/crates/parity/regression.json`
- Parity script: `scripts/parity.mjs`
- Parity binary: `manabrew-rs/crates/parity/src/main.rs`
