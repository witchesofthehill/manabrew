...
8. **Keep default identity and action cues deliberate.** The Manabrew preset has three brand colors: coral primary with charcoal text, berry secondary with white text, and sage-green accent with charcoal text. Structural backgrounds, muted surfaces, borders, and inputs stay neutral. Solid primary buttons use coral only for the action that advances the current decision; secondary buttons use restrained berry tint, outline and ghost hover states stay neutral, and selected controls use the app `selection` token. Accent is not another action-priority level. Focus rings, priority, `cardRing`, and `activeAction.active` use primary; selection uses secondary. Player identity and rules-significant colors remain distinct where gameplay requires them. Text and icons on solid player-color fills must use `readableTextColor`; never assume `textOnTinted` is legible on every seat hue. Warning remains separate from the brand palette.
