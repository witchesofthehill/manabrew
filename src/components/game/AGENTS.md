...
- **Layout** is a single arrangement: the local player along the bottom (`RegionOrientation` `bottom`), opponents across the top (`top`). Each opponent field's `rect` is the **fixed play area** — `computeBoardLayout` makes every field `L - n·x` wide (canvas minus one un-collapsible banner per field) and **staggers** them by a constant step so the `n` fields span the canvas. The grid, card positions, felt and playmat are all drawn from this fixed `rect` and **never move or reflow**.
...
- **Opponent player bar (experimental, `pixiPlayerBar` pref)**: when on, each opponent's React `PlayerPanel` is hidden and a thin Pixi bar (`PlayerBarLayer`, `PLAYER_BAR_HEIGHT_PX`) is drawn across the top of their clip band (name + life + seat-colour border + tap-to-target). `BoardScene` positions the bars from the live delimiters and reserves the bar's height at the top of the opponent grid via `RegionHost.getTopReserve`, so the first card row sits below the bar. Driven by `BoardCanvas`'s `playerBars` / `showPlayerBars` props. Self still uses the React panel. Deferred: avatar, zone tiles, badges, a self bar.
