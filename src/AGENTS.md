...
- **Usernames may carry a collision tag.** When the relay rejects a login as a duplicate, `platform/web.ts` retries under a `name@NNNN` tag (`suffixUsername`, bounded by `MAX_DUPLICATE_NAME_RETRIES`) instead of surfacing the error, and the store adopts the tagged name via `AuthResultPayload.username`. The tag is part of the identity — relay routing, seat matching, and `roomByName` lookups all use the full tagged string — so strip it **only at render time** with `stripUsernameTag` (`lib/username.ts`); never strip it before a comparison or a launch payload.
