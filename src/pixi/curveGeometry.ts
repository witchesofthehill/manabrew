...
export function cubicAt(curve: CubicCurve, t: number): Point {
  const u = 1 - t;
  const uu = u * u;
  const tt = t * t;
  return {
...
export function sampleCubic(curve: CubicCurve, steps: number): Point[] {
  const points: Point[] = [];
  for (let i = 0; i <= steps; i += 1) points.push(cubicAt(curve, i / steps));
  return points;
}
