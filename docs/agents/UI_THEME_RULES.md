...
8. **Keep default identity and action cues deliberate.** The Manabrew preset uses dark orange with white text for app primary actions, while `cardRing` and `activeAction.active` use the brighter brand orange. Sage owns secondary controls, peach owns accent surfaces, and selection uses forest green in light mode and sage in dark mode. Priority inherits the app primary pair. Player identity colors remain distinct green, blue, yellow, and pink hues. Text and icons on solid player-color fills must use `readableTextColor`; never assume `textOnTinted` is legible on every seat hue. Warning remains separate from primary orange. Graveyard accents use the preset's neutral sage.
