...
//! a plane that never starts, so a build carries no QUIC stack and no
//! second listening socket until someone asks for it.
...
impl DirectPlane {
    ...
    pub async fn adopt_relay(&self, _url: Option<&str>) -> bool {
        match self.0 {}
    }
