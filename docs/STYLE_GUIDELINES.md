...
Pixi engine prompts use the matching `PromptLayer.createModalShell` composition. Every modal prompt type supplies stage content and footer controls to that shared shell. Text-bearing header and footer chrome uses the near-opaque themed card surface, canvas shadow, and foreground text so the battlefield cannot reduce legibility. Keep main footer actions right-aligned, with cancel or deny immediately before confirm; utility actions may remain on the left. When a prompt has `sourceCard`, treat the prompt, standard source gap, and interactive source preview as one centered screen cluster. Apply this placement to every prompt type so the source remains next to the minimize control regardless of prompt width. Align the source and prompt top edges, and keep the rail independent of the prompt's scroll and interaction layout. On viewports too narrow for a usable prompt beside the rail, center the prompt alone and stack the same preview-sized card in the header. Scry uses the full remaining workspace width and sizes its two card rows from the shell's measured body height so standard desktop viewports do not scroll. All prompt cards use `GAME_CARD_SIZES.preview` through `promptCardDimensions`; scale down only when the viewport, destination zone, or item density requires it.

`chooseCards` and `revealCards` use the zone browser's centered overlapping card ribbon rather than a grid. Preserve its wheel and trackpad movement, touch swipe, focus spread and lift, card elevation, feedback easing, snap timing, active ring, and Left/Right Arrow navigation. The zone scene and prompt renderer both consume `ZONE_BROWSER_RIBBON_MOTION`; never approximate those values separately. Arrow handling prevents the browser default and stops further key propagation. Choosing a card may change selection; revealing a card only changes the active card until Continue is pressed.
