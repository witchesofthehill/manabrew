...
export function tableTexture(colors: ArenaColors) {
  ...
  for (let y = 0; y < 1400; y += 64)
    for (let x = 0; x < 1536; x += 96) {
      ctx.globalAlpha = 0.1 + ((x * 13 + y * 7) % 19) / 150;
      ctx.fillStyle = (x + y) % 128 ? colors.background : colors.border;
  ...
  light.addColorStop(0, "rgba(110,140,160,.12)");
  light.addColorStop(0.6, "rgba(12,18,28,.08)");
  light.addColorStop(1, "rgba(4,8,16,.68)");
  ctx.globalAlpha = 1;
  ctx.fillStyle = light;
