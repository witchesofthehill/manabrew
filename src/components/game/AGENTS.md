...
- **Pixi player bar + on-grid zones (experimental, `pixiPlayerBar` pref)**: when on, every React `PlayerPanel` is hidden and replaced by Pixi.
  - **`PlayerBarLayer`** draws a thin bar per player: a circular **avatar** (photo → bot game-icon → name initial; the left-edge cap), a red ♥ + **life**, seat-colour ring (active-turn cue). Self bar is bottom-left (`SELF_PLAYER_BAR_HEIGHT_PX`); opponent bars across the top of their clip band, eased with the delimiters, fixed width (`PLAYER_BAR_MAX_WIDTH_PX`, overflows a narrow field) and collapsing to an avatar **sphere** when the field is at banner width. Top/side margins; the opponent grid reserves the bar height via `RegionHost.getTopReserve`. Tap the avatar to target. Driven by `BoardCanvas` `playerBars` / `showPlayerBars`.
  - **`BoardZoneTiles`** (owned by each `BoardRegion`) renders the **deck (card back) / graveyard / exile / command** zones as cards **on the grid**, occupying grid cells (bottom-left by default via `BoardRegion.placeZoneTiles`, which reserves those cells so battlefield cards lay out around them). A non-empty zone shows the top card's `CardSprite` face (the MTG card back for the deck, loaded once via the shared `loadCardBack` memo so every region gets it) centred in the cell + a count pill; an **empty** zone is a dotted rounded-rect outline with the zone glyph (`ZONE_ICONS`). No labels. Tap-to-open + playable/target highlight for **all** players (same behaviour as the old panel); the **local** player can drag a tile to another cell (drags paint the drop grid via `BoardRegion.drawDropGrid`, snap + persist in `BoardRegion.zoneSlots`), opponents are static. Driven by `BoardCanvas` `zoneTiles` (a `Record<playerId, ZoneTileSpec[]>`) → `BoardScene.setZoneTiles`.
- `GameBoard` anchors its React player panels to each field via `BoardCanvas`'s `onLayout` (which now carries only `rect`/`orientation`, no clip). Every player's React panel is measured (relative to the board) and pushed to its region via `BoardScene.setPlayerBlockers` as a canvas-space keep-out, so battlefield cards never lay out under that player's own zones/avatar — converted into the region's local space (`collectLocalBlockers`).
