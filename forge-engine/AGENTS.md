...
| `forge-server`          | Standalone matchmaking/lobby server. Optional. Caches in-game state per room (`replay.rs`) so reconnecting clients can pull a `RequestResync` replay (GameStarted + last state + pending prompt); the room's `reconnect_timeout_s` is clamped ≤ 90s to stay under the engine's 120s auto-pass (`forge-game-runtime/src/mpsc_transport.rs`). | —                                            |
